...
Generate only the caption text, no additional commentary.`;
    ...
    const command = new InvokeModelCommand({
      modelId: process.env.BEDROCK_CLAUDE_MODEL_ID || 'anthropic.claude-3-5-sonnet-20240620-v1:0',
...
async function generateAndUploadImage(brandData, contentPillar, caption, postId) {
  try {
    ...
    const imagePrompt = `Create a high-quality social media image with the following characteristics:
...
Resolution: 1080x1080 pixels (square format).`;
    ...
    const command = new InvokeModelCommand({
      modelId: process.env.BEDROCK_TITAN_MODEL_ID || 'amazon.titan-image-generator-v1',
      body: JSON.stringify(requestBody),
    });
    ...
    if (!responseBody.images || responseBody.images.length === 0) {
      throw new Error('No image generated by Titan');
    }
