...
#
# Usage: .\scripts\clear-dev-mode-localstorage.ps1
...
# JavaScript code to clear localStorage
$jsCode = @'
...
console.log('%c🧹 Clearing Development Mode localStorage...', 'color: #f59e0b; font-size: 16px; font-weight: bold;');
console.log('');

let clearedCount = 0;
itemsToClear.forEach(key => {
    if (localStorage.getItem(key) !== null) {
        console.log('%c✓ Cleared: ' + key, 'color: #10b981;');
        localStorage.removeItem(key);
        clearedCount++;
    } else {
        console.log('%c- Not found: ' + key, 'color: #6b7280;');
    }
});

console.log('');
console.log('%c✓ Cleared ' + clearedCount + ' items', 'color: #10b981; font-weight: bold;');
console.log('');
console.log('%c🔄 Reloading page...', 'color: #3b82f6; font-size: 14px;');
...
$encodedJs = [System.Uri]::EscapeDataString($jsCode)

# Create bookmarklet URL
$bookmarkletUrl = "javascript:$encodedJs"
...
<body>
    ...
    <script>
        ...
        function log(message, color = '#1f2937') {
...
'@
    ...
    Write-Host "2. Navigate to: $devServerUrl/app" -ForegroundColor White
